...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /* ---------- ABOUT: letter-by-letter headline reveal ---------- */
  (function aboutHeadlineSplit(){
    const heading = document.getElementById('aboutHeading');
    if (!heading) return;

    if (reduceMotion){
      heading.classList.add('letters-in');
      return;
    }
  ...
  /* ---------- ABOUT: glass-card cursor-follow glow ---------- */
  (function aboutCardGlow(){
    if (isTouch) return;
    document.querySelectorAll('#about .glass-card').forEach(card => {
  ...
  /* ---------- ABOUT: technology orbit — activate once visible ---------- */
  (function techOrbitActivate(){
    const orbit = document.getElementById('techOrbit');
    if (!orbit) return;
    if (reduceMotion){ orbit.classList.add('is-active'); return; }
    const oio = new IntersectionObserver((entries) => {
      entries.forEach(e => {
        if (e.isIntersecting){ orbit.classList.add('is-active'); oio.disconnect(); }
      });
    }, { threshold: 0.3 });
    oio.observe(orbit);
  })();
